function pipeline_proc_fits, work_dir $
                    , fps = fps $
                    , no_save_empty = no_save_empty, no_visual = no_visual, no_cand = no_cand, no_details = no_details $
                    , presets_file = presets_file $
                    , method = method, graphtype = graphtype $
                    , use_jpg = use_jpg, use_contour = use_contour $
                    , test = test 
...
pipeline_proc_dir_tree, work_dir, config, aia_dir_cache, event_rel, aia_dir_wave_sel, obj_dir, vis_data_dir, vis_data_dir_wave $
...
foreach wave, config.waves, i do begin
    ...
    if keyword_set(remote_cutout) then begin
        ...
        if ~keyword_set(no_load) then begin
            code = aia_download_by_config(wave, aia_dir_cache, config, dirmode = 'cache')
            if code lt 0 then continue
        endif
        if ~keyword_set(no_cut) then begin
            code = pipeline_aia_cutout(aia_dir_cache, work_dir, wave, aia_dir_wave_sel[i], config, nofits = nofits, sav = sav)
            if code lt 0 then continue
        endif
